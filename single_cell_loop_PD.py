...
def main():
    ...
    if cold_core_flag < 6 and rain_flag < 6: ##COLD CORE AND PRECIP TO PERSIST FOR AT LEAST 6 HRS OF THE CELLS LIFETIME ##
        ...
        subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/CPM_PD_TRACKS/final_tracks_INTERP/{}/deleted_tracks/both/tracks_cell_{}.hdf'.format(year, cell), 'table')

    else:
        if cold_core_flag < 6:
            subset = subset.drop(subset[subset.cell == cell].index)
            subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/CPM_PD_TRACKS/final_tracks_INTERP/{}/deleted_tracks/cold_core/tracks_cell_{}.hdf'.format(year, cell), 'table')
        elif rain_flag < 6:
            subset = subset.drop(subset[subset.cell == cell].index)
            subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/CPM_PD_TRACKS/final_tracks_INTERP/{}/deleted_tracks/precip/tracks_cell_{}.hdf'.format(year, cell), 'table')
        else:
            subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/CPM_PD_TRACKS/final_tracks_INTERP/{}/CC&PF/tracks_cell_{}.hdf'.format(year, cell), 'table')
