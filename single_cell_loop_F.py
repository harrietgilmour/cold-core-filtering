...
def open_datasets(mask_file, precip_file, tracks_file, tb_file, w_file):
    ...
    precip = precip.__xarray_dataarray_variable__
    ...
    vert_vel = vert_vel.squeeze('p') # removes p dimesion
    vert_vel = vert_vel[:,1:,:]
...
def add_CC_PF_columns(tracks):
    ...
    # Add column for datetime (CFTIME FOR 360 DAY CALENDARS)
    #tracks['cf_time'] = 0
...
def main():
    ...
    if cold_core_flag < 6 and rain_flag < 6: ##COLD CORE AND PRECIP TO PERSIST FOR AT LEAST 6 HRS OF THE CELLS LIFETIME ##
        ...
        subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/CPM_F_TRACKS/final_tracks/orig_threshold/{}/deleted_tracks/both/tracks_cell_{}.hdf'.format(year, cell), 'table')

    else:
        if cold_core_flag < 6:
            subset = subset.drop(subset[subset.cell == cell].index)
            subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/CPM_F_TRACKS/final_tracks/orig_threshold/{}/deleted_tracks/cold_core/tracks_cell_{}.hdf'.format(year, cell), 'table')
        elif rain_flag < 6:
            subset = subset.drop(subset[subset.cell == cell].index)
            subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/CPM_F_TRACKS/final_tracks/orig_threshold/{}/deleted_tracks/precip/tracks_cell_{}.hdf'.format(year, cell), 'table')
        else:
            subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/CPM_F_TRACKS/final_tracks/orig_threshold/{}/CC&PF/tracks_cell_{}.hdf'.format(year, cell), 'table')
