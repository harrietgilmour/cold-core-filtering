...
# This script differs to single_cell_loop.py as it contains a loop that loops over cells in this python file 
# and removes the need for an additional bash script. It also means that only 1 job is submitted per month
# rather than hundreds.
# THIS  SCRIPT USES DASK TO PARALLELISE THE PROCESS OF RUNNING THROUGH EACH CELL WITHIN EACH MONTH THAT IS SUBMITTED
#
# <USAGE> python single_cell_loop.py <YEAR> <MONTH>
#
# <EXAMPLE> python single_cell_loop.py 2005 01
...
def check_no_args(args):
    ...
    if len(args) != 3:
        ...
        print('Usage: python single_cell_loop.py <YEAR> <MONTH')
        print('Example: python single_cell_loop.py 2005 01')
        sys.exit(1)


# WRITE A FUNCTION THAT FINDS THE RIGHT DATASETS FOR THE SPECIFIED MONTH AND YEAR
def find_datasets(year, month):
    print(year)
    print(month)
    # set up the mask, precip and tracks file
    mask_dir="/data/users/hgilmour/initial_tracks/tobac_initial_tracks/segmentation"
    mask_name="segmentation_yearly_{}.nc".format(year) # a year because these ad the tracks were done in yearly chunks so frame numbers are also based on year

    precip_dir="/scratch/hgilmour/total_precip/yearly_files"
    precip_name="total_precip_{}.nc".format(year)

    tracks_dir="/data/users/hgilmour/initial_tracks/tobac_initial_tracks/tracking"
    tracks_name="tracks_{}_{}.h5".format(year, month)

    tb_dir="/data/users/hgilmour/tb"
    tb_name="tb_{}.nc".format(year)

    w_dir="/scratch/hgilmour/omega_new"
    w_name="omega_{}.nc".format(year)

    # form the file paths
    mask_file = mask_dir + '/' + mask_name
    print("Mask filepath:", mask_file)
    precip_file = precip_dir + '/' + precip_name
    tracks_file = tracks_dir + '/' + tracks_name
    tb_file = tb_dir + '/' + tb_name
    w_file = w_dir + '/' + w_name

    return mask_file, precip_file, tracks_file, tb_file, w_file
...
# WRITE A FUNCTION THAT FINDS THE UNIQUE CELL VALUES FOR THAT MONTH AND YEAR
def find_unique_cells(year, month):
    txt_file_path="/data/users/hgilmour/cold-core-filtering/unique_cell_files"
    txt_file_name="unique_cells_{}_{}.txt".format(year,month)
    # form the file path
    txtfile = txt_file_path + '/' + txt_file_name
    # extract array of values from txtfile
    with open(txtfile, 'r') as f:
        cell_numbers=f.readlines()
        cell_numbers = [int(i.strip()) for i in cell_numbers]
        print(cell_numbers)
...
def process_mcs_cell(params):
    ...
    if cold_core_flag < 6 and rain_flag < 6: ##COLD CORE AND PRECIP TO PERSIST FOR AT LEAST 6 HRS OF THE CELLS LIFETIME ##
        ...
        subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/final_tracks_CPM/{}/deleted_tracks/both/tracks_cell_{}.hdf'.format(year, cell), 'table')
        save_folder = 'deleted_tracks/both'

    else:
        if cold_core_flag < 6:
            subset = subset.drop(subset[subset.cell == cell].index)
            subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/final_tracks_CPM/{}/deleted_tracks/cold_core/tracks_cell_{}.hdf'.format(year, cell), 'table')
            save_folder = 'deleted_tracks/cold_core'
        elif rain_flag < 6:
            subset = subset.drop(subset[subset.cell == cell].index)
            subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/final_tracks_CPM/{}/deleted_tracks/precip/tracks_cell_{}.hdf'.format(year, cell), 'table')
            save_folder = 'deleted_tracks/precip'
        else:
            subset.to_hdf('/project/cssp_brazil/mcs_tracking_HG/final_tracks_CPM/{}/CC&PF/tracks_cell_{}.hdf'.format(year, cell), 'table')
            save_folder = 'CCPF'
...
#Extract year and month from the command line
year = str(sys.argv[1])
print(year)
